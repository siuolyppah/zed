...
use workspace::{
    ...
    item::ItemHandle,
    ui::{
        h_flex, v_flex, ActiveTheme, ContextMenu, FluentBuilder, Icon, IconName, IconSize, Label,
        LabelCommon, ListItem, Selectable, StyledTypography,
    },
...
impl OutlinePanel {
    ...
    fn new(workspace: &mut Workspace, cx: &mut ViewContext<Workspace>) -> View<Self> {
        ...
        let outline_panel = cx.new_view(|cx| {
            ...
            let delegate = OutlinePanelDelegate::new(project.clone(), cx);
...
impl FocusableView for OutlinePanel {
    fn focus_handle(&self, cx: &AppContext) -> FocusHandle {
        self.picker.read(cx).delegate.focus_handle.clone()
    }
}
            ...
            h_flex()
                ...
                .on_mouse_down(
                    ...
                    cx.listener(move |outline_panel, event: &MouseDownEvent, cx| {
                        outline_panel.picker.update(cx, |picker, cx| {
                            if let Some(entry) = picker.delegate.selected_entry.clone() {
                                picker.delegate.deploy_context_menu(
                                    event.position,
                                    entry.to_ref_entry(),
                                    cx,
                                )
                ...
                .children(self.picker.read(cx).delegate.context_menu.as_ref().map(
                    |(menu, position, _)| {
                        deferred(
                            anchored()
                                .position(*position)
...
fn subscribe_for_editor_events(
    editor: &View<Editor>,
    cx: &mut ViewContext<'_, Picker<OutlinePanelDelegate>>,
) -> Subscription {
    let debounce = Some(Duration::from_millis(UPDATE_DEBOUNCE_MILLIS));
    cx.subscribe(editor, move |picker, editor, e: &EditorEvent, cx| match e {
        EditorEvent::SelectionsChanged { local: true } => {
            picker.delegate.reveal_entry_for_selection(&editor, cx);
            cx.notify();
        }
...
fn is_unfoldable(entry: EntryRef) -> bool {
...
impl OutlinePanelDelegate {
    pub fn new(project: Model<Project>, cx: &mut ViewContext<'_, OutlinePanel>) -> Self {
        let focus_handle = cx.focus_handle();
        Self {
            project,
            focus_handle,
            ...
            .child(
                ListItem::new(item_id)
                    ...
                    .on_click({
                        ...
                        cx.listener(move |picker, event: &gpui::ClickEvent, cx| {
                            if event.down.button == MouseButton::Right || event.down.first_mouse {
                                return;
                            }
                            picker.delegate.open_entry(&clicked_entry, cx);
                        })
                    })
                    .on_secondary_mouse_down(cx.listener(
                        move |picker, event: &MouseDownEvent, cx| {
    ...
    fn deploy_context_menu(
        ...
        let is_unfoldable = auto_fold_dirs && !is_root && is_unfoldable(entry);
        ...
        let subscription = cx.subscribe(&context_menu, |picker, _, _: &DismissEvent, cx| {
            picker.delegate.context_menu.take();
            cx.notify();
        });
    ...
    fn fetch_outlines(&mut self, range: &Range<usize>, cx: &mut ViewContext<'_, Picker<Self>>) {
    ...
    > {
        ...
        self.entries_with_depths(cx)
            ...
            .into_iter()
            .flatten()
            .fold(HashMap::default(), |mut excerpts_to_fetch, (_, entry)| {
                match entry {
...
impl PickerDelegate for OutlinePanelDelegate {
    ...
    type ListItem = Stateful<Div>;

    fn match_count(&self) -> usize {
        // TODO kb have to be real matches
    ...
    fn render_match(
        ...
        let (depth, entry) = self.cached_entries_with_depth.as_ref()?.get(ix)?;
        match entry {
            EntryOwned::Entry(entry) => Some(self.render_entry(&entry, *depth, cx)),
            EntryOwned::FoldedDirs(worktree_id, entries) => {
                Some(self.render_folded_dirs(*worktree_id, &entries, *depth, cx))
            }
            EntryOwned::Excerpt(buffer_id, excerpt_id, excerpt) => {
                self.render_excerpt(*buffer_id, *excerpt_id, &excerpt, *depth, cx)
            }
            EntryOwned::Outline(buffer_id, excerpt_id, outline) => {
                Some(self.render_outline(*buffer_id, *excerpt_id, outline, *depth, cx))
            }
        }
    }
}
